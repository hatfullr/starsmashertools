...
class Binary(starsmashertools.lib.simulation.Simulation, object):
    ...
    def _animation_update(self, output, artist, *args, **kwargs):
        ret = super(Binary, self)._animation_update(output, artist, *args, **kwargs)
        ...
        if artist.get_array() is None:
            ...
            color_dict = {
                'Primary'   : np.array([0, 0, 0]), # Black
                'Secondary' : np.array([1, 0, 0]), # Red
            }
            ...
            _colors = np.zeros(output['ntot'])
            _colors[secondary] = 1 # red
            artist.set_array(_colors)
            
            # Make a legend to explain
            patches = []
            for label, color in color_dict.items():
                patches += [matplotlib.lines.Line2D(
                    [0],[0],
                    marker = 'o',
                    color = 'w',
