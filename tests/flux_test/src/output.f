...
c            call mpi_gatherv(rho(n_lower), mylength, mpi_double_precision,
c     $           rho, recvcounts, displs, mpi_double_precision, 0,
c     $           mpi_comm_world, ierr) 
c            call mpi_gatherv(divv(n_lower), mylength, mpi_double_precision,
c     $           divv, recvcounts, displs, mpi_double_precision, 0,
c     $           mpi_comm_world, ierr) 
c         else
...
c         endif
     ...
     $        mpi_comm_world, ierr)
         call mpi_allgatherv(mpi_in_place, mylength, mpi_double_precision,                                   
     $        divv, recvcounts,displs, mpi_double_precision,
     $        mpi_comm_world, ierr)
...
c     IF EDITS ARE MADE IN THIS ROUTINE:
c        Check to make sure init.f matches (~L172)
...
c     $              (etot-einit)/abs(einit), stot,ajtot,
     ...
     $                 omeg,
     $                 sqrt((xcm1-xcm2)**2+(ycm1-ycm2)**2+(zcm1-zcm2)**2)
               else if(ncooling.eq.1.or.ncooling.eq.2.or.
     $                 ncooling.eq.3) then
                  write(22,'(14g15.7)')t,epot,ekin,eint,etot,stot,ajtot,
     $                 erad,omeg,
     $                 sqrt((xcm1-xcm2)**2+(ycm1-ycm2)**2+(zcm1-zcm2)**2)
...
c     $              (etot-einit)/abs(einit), stot,ajtot
               if(ncooling.eq.0) then
                  write(22,'(14g15.7)')t,epot,ekin,eint,etot,stot,ajtot
               else if(ncooling.eq.1.or.ncooling.eq.2
     $                 .or.ncooling.eq.3) then
                  write(22,'(14g15.7)')t,epot,ekin,eint,etot,stot,ajtot,
     $                 erad
               else
                  if (myrank.eq.0) then
...
c     this is here for the generation of col*.sph files
     ...
     $           mpi_comm_world, ierr)
      ...
      if(myrank.eq.0) then
         write(outfn,fileformat) nout
         
c         if(nout.lt.10000) then
c            write(outfn,1101) nout
c 1101       format('out',i4.4,'.sph')
c         else
c            write(outfn,2101) nout
c 2101       format('out',i5.5,'.sph')
c         endif
         
         write (69,*) 'duout: writing file ',trim(adjustl(outfn)),
     $        'at t=',t
         
         open(12,file=trim(adjustl(outfn)),form='unformatted')
         call dump(12)
         close (12)
      endif
...
c               if(myrank.ne.0)then
c                  call mpi_gatherv(gx(ngrav_lower), mygravlength, mpi_double_precision,
c     $                 gx, gravrecvcounts, gravdispls, mpi_double_precision, 0,
c     $                 comm_worker, ierr)
c                  call mpi_gatherv(gy(ngrav_lower), mygravlength, mpi_double_precision,
c     $                 gy, gravrecvcounts, gravdispls, mpi_double_precision, 0,
c     $                 comm_worker, ierr)
c                  call mpi_gatherv(gz(ngrav_lower), mygravlength, mpi_double_precision,
c     $                 gz, gravrecvcounts, gravdispls, mpi_double_precision, 0,
c     $                 comm_worker, ierr)
c               else
...
c               endif
     ...
     $              gx, gravrecvcounts,gravdispls, mpi_double_precision,                                     
     $              comm_worker, ierr)
               call mpi_allgatherv(mpi_in_place, mygravlength, mpi_double_precision,                         
     $              gy, gravrecvcounts,gravdispls, mpi_double_precision,                                     
     $              comm_worker, ierr)
               call mpi_allgatherv(mpi_in_place, mygravlength, mpi_double_precision,                         
     $              gz, gravrecvcounts,gravdispls, mpi_double_precision,                                     
     $              comm_worker, ierr)
            endif
         endif

         if(myrank.eq.0) then
            write(outfn,fileformatcol) nout
c            write(outfn,102) nout
c 102        format('col',i4.4,'.sph')
