     ...
     $     icomp
      ...
      if(u(no).ne.0.d0)then
         stop 'Code assumes that last particle is a black hole...' 
      endif
      do i=1,no-1               ! no-1: do not include the black hole yet
         ...
         if(ribh2.ge.reat**2) then
            ...
            if(ncooling.eq.1) then
               ...
               uraddot(ntot)=uraddot(i)
               temperatures(ntot)=temperatures(i)
               popacity(ntot)=popacity(i)
               tau(ntot)=tau(i)
               dEemergdt(ntot)=dEemergdt(i)
               dEdiffdt(ntot)=dEdiffdt(i)
               dEmaxdiffdt(ntot)=dEmaxdiffdt(i)
               rout(ntot)=rout(i)
               ...
               dEmaxdiffdt(ntot)=dEmaxdiffdt(i)
               rout(ntot)=rout(i)
               uraddotcool(ntot)=uraddotcool(i)
               uraddotheat(ntot)=uraddotheat(i)
            end if
         else
...
c     always keep the black hole:
      ...
      if(ncooling.eq.1)then
         ...
         uraddot(ntot)=0d0
         temperatures(ntot)=0d0
         popacity(ntot)=0d0
         tau(ntot)=0d0
         dEemergdt(ntot)=0d0
         dEdiffdt(ntot)=0d0
         dEmaxdiffdt(ntot)=0d0
         rout(ntot)=0d0
      else if(ncooling.eq.3) then
         uraddot(ntot)=0d0
         temperatures(ntot)=0d0
         ...
         dEmaxdiffdt(ntot)=0d0
         rout(ntot)=0d0
         uraddotcool(ntot)=0d0
         uraddotheat(ntot)=0d0
      end if
