...
!         amnotte=mass not in thermal equilibrium
      ...
      if(ndisplace.ne.0) then
         ...
         if(u(n).ne.0) then
            ...
            stop 'Assumed last particle was black hole. See log.'
         endif
...
!               ttherm=(ueq(i)-u(i))/uraddoti
            ...
            else if(ncooling.eq.2.or.ncooling.eq.3) then
               u(i)= u(i) + udot(i) * dth
            else
               if (myrank.eq.0) then
                  write(*,*) "Unimplemented ncooling value ",ncooling
                  error stop "advance.f90"
               end if
            end if
            ...
            stop 'Black hole particle has udot =/= 0. See log.'
         endif
...
!               endif
               ...
               uorig=uo(i)+(dth+dthnew)*udot(i)
               u(i)= uo(i) + (udot(i)+uraddot(i))*(dth+dthnew)

               if (myrank.eq.0 .and. u(i).le.0.d0) then
                  write(*,*) "Particle",i,"has u <= 0"
                  write(*,*) "u before cooling = ",uo(i)
                  write(*,*) "u after cooling  = ",u(i)
                  write(*,*) "x, y, z = ",x(i),y(i),z(i)
                  write(*,*) "am, rho, hp = ",am(i),rho(i),hp(i)
                  write(*,*) "vx, vy, vz = ",vx(i),vy(i),vz(i)
                  write(*,*) "vxdot, vydot, vzdot = ",vxdot(i),vydot(i),vzdot(i)
                  write(*,*) "udot, uraddot = ",u(i),udot(i),uraddot(i)
                  write(*,*) "grpot, meanmolecular = ",grpot(i),meanmolecular(i)
                  write(*,*) "cc, ueq, tthermal = ",cc(i),ueq(i),tthermal(i)
                  write(*,*) "popacity, temperature, tau = ",popacity(i),temperatures(i),tau(i)
                  error stop "advance.f90"
               end if
      ...
      if(myrank.eq.0 .and. ncooling.eq.1)&
...
!      enddo
      call mpi_allgatherv(mpi_in_place, mylength, mpi_double_precision,&
           hp,recvcounts,displs,mpi_double_precision,mpi_comm_world,ierr)
