...
c     initialization of run (new or restart)
     ...
     $     alphaold,betaold,trelaxold
      ...
      character*255 fileformat, fileformatcol
      common/fileform/fileformat, fileformatcol
      logical fileexists
      integer ios
      real*8 temp_cuf, temp_tcool
...
c     get parameters from input file:
      ...
      write(fileformat,'("(",A,",i",I0,".",I0,",",A,")")')
     $     "'out'",
     $     int(log10(tf/dtout))+1,
     $     int(log10(tf/dtout))+1,
     $     "'.sph'"
      write(fileformatcol,'("(",A,",i",I0,".",I0,",",A,")")')
     $     "'col'",
     $     int(log10(tf/dtout))+1,
     $     int(log10(tf/dtout))+1,
     $     "'.sph'"
...
c     compute look-up tables:
     ...
     $     nav,nintvar,neos,ncooling
      ...
      if(ncooling.eq.2 .or. ncooling.eq.3) then
         call init_cooling
      end if
...
c     used in subroutine dump)
     ...
     $           ngrold,nrelaxold,trelaxold,dt,omega2
         ...
         if ( (ncooling.eq.2.or.ncooling.eq.3).and.
     $        (ncoolingold.eq.2.or.ncoolingold.eq.3) ) then
            inquire(file='uraddotupdates.dat', exist=fileexists)
            if(fileexists) then
               open(699, file='uraddotupdates.dat', status='old')
               do
                  read(699, *, iostat=ios) temp_tcool,temp_cuf
                  if(ios.ne.0) exit
                  if(temp_tcool.gt.t) exit
                  tcool = temp_tcool
                  cooling_update_frequency = temp_cuf
               end do
               close(699)
            else
               error stop "You must give uraddotupdates.dat when "//
     $              "restarting a run using a restartrad.sph file "//
     $              "with ncooling=2 or ncooling=3"
            end if
         end if
...
c         endif
     ...
     $              ueq(i),tthermal(i)
            enddo
         else if(ncoolingold.eq.2) then
            do i=1,ntot
               read(12) x(i),y(i),z(i),am(i),hp(i),rho(i),
     $              vx(i),vy(i),vz(i),vxdot(i),vydot(i),vzdot(i),
     $              u(i),udot(i), !gx(i),gy(i),gz(i),
     $              grpot(i),meanmolecular(i),cc(i),divv(i),
     $              ueq(i),tthermal(i),popacity(i),uraddot(i),
     $              temperatures(i),tau(i),dEemergdt(i),dEdiffdt(i),
     $              dEmaxdiffdt(i)
            enddo
         else if(ncoolingold.eq.3) then
            do i=1, ntot
               read(12) x(i),y(i),z(i),am(i),hp(i),rho(i),
     $              vx(i),vy(i),vz(i),vxdot(i),vydot(i),vzdot(i),
     $              u(i),udot(i), !gx(i),gy(i),gz(i),
     ...
     $              ueq(i),tthermal(i),popacity(i),uraddot(i),
     $              temperatures(i),tau(i),dEemergdt(i),dEdiffdt(i),
     $              dEmaxdiffdt(i),uraddotcool(i),uraddotheat(i)
            end do
         else
            if (myrank.eq.0) then
               write(*,*) "Unimplemented ncooling value ",ncooling
               error stop "init.f"
            end if
         end if
         read(12) nchk
...
c     get 3-letter code for type of initial condition from init file
         ...
         elseif(iname.eq.'txt')then
            ...
            if(myrank.eq.0) write(69,*)
     $           'init: unknown name. iname = ',iname
            stop 'init: unknown name. See log.'
         endif
      endif

      if(n.gt.nmax)then
         if(myrank.eq.0) write(69,*)'init: increase nmax to',n
         stop 'init: increase nmax'
      endif
 ...
 101     format (' init: parameters for this ideal + radiation run:',/,
     ...
     $        ' corepts=',i2,/)
         ...
         elseif(nintvar.eq.2)then
            ...
            stop 'must integrate either a or u'
         endif
         if(neos.eq.0) then
     ...
     $           displacex,displacey,displacez
      ...
      if(ncooling.eq.2.or.ncooling.eq.3 .and. myrank.eq.0) then
         inquire(file='uraddotupdates.dat', exist=fileexists)
         if(fileexists) then
            open(699, file='uraddotupdates.dat', status='old',
     $           position='append')
         else
            open(699, file='uraddotupdates.dat', status='unknown')
         end if
         write(*,*) "tcool, next tcool = ",tcool,
     $        tcool+cooling_update_frequency
      end if
      

      if(myrank.eq.0) then
         write(69,*) "init completed successfully"
         call flush(69)
      end if
      return
...
************************************************************************
     ...
     $     stellarevolutioncodetype,icosahedron_subdivisions,
     $     thetawindow,phiwindow,max_cooling_update_frequency,
     $     cooling_type
...
!     the final timestep dt is the minimum of dt_sph and dt_co for all particles i
      ...
      starmass=1d0
      starradius=1d0
      ncooling=0                ! 0 if no cooling, otherwise radiative cooling. 1 for Lombardi et. al (2015) 10.48550/arXiv.1411.3678, 2 for diffusion-limited radiative cooling, 3 for diffusion-limited radiative cooling AND heating (publication to come [hopefully!]...)
      nkernel=0
      teq=100d0
      tjumpahead=1d30
      ...
      if(startfile2.ne.'sph.start2u') then
         ...
         if(.not. fileexists) then
            ...
            stop 'startfile2 given in sph.input does not exist'
         endif
      endif

      if ( myrank.eq.0 .and.
     $     (ncooling.eq.1.or.ncooling.eq.2.or.ncooling.eq.3))
     $     write(69,*) 'background temperature teq=',teq
...
c     omega_spin>0 gives counterclockwise spin:
      ...
      if ( ncooling.eq.2.or.ncooling.eq.3 ) call kappa_and_T
...
c     used in subroutine dump)
     ...
     $        cc(i)
         if(u(i).eq.0.d0) then
            corepts=corepts+1
            stop 'should corepts really be set properly?'
         endif
      enddo
      read(12) nchk
      close(12)
